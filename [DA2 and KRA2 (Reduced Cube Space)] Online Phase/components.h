...
struct TinyJambu {
    ...
    TinyJambu(int, int, int);
    uint32_t state_update(uint32_t *, const uint8_t *, uint32_t);
    uint32_t last_state_update(uint32_t *, const uint8_t *, uint32_t);
...
TinyJambu::TinyJambu(int NROUND1 = 384, int NROUND2 = 1024, int NROUND3 = 1024)
{
    this -> NROUND1 = NROUND1;
    this -> NROUND2 = NROUND2;
    this -> NROUND3 = NROUND3;

//    std::cout << std::endl << "Rounds: " << (NROUND3 & 31);
}
...
uint32_t TinyJambu::last_state_update(uint32_t *state, const uint8_t *key, uint32_t number_of_steps)
{
	uint32_t i;
	uint32_t t1, t2, t3, t4, feedback;
	for (i = 0; i < (number_of_steps >> 5); i++)
...
uint32_t TinyJambu::initialization(const uint8_t *key, const uint8_t *iv, uint32_t *state)
{
    ...
    state[1] ^= FrameBitsPC;
    ks = last_state_update(state, key, NROUND3);

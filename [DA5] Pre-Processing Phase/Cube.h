...
class Cube
{
    public:
        ...
        Cube(int, int, int, int, int, int, int);
        ~Cube();
...
/** constructor **/
Cube::Cube(int cubesNo = 0, int ltNo = 0, int lc = 0, int mlen = 0, int adlen = 0, int NROUND1 = 384, int NROUND2 = 1024)
{
...
void Cube::startAttack()
{
    ...
    for(int i = 0; i < cubesNo; i++)
    {
        ...
        std::cout << std::endl << "Associated Data Length (bytes)\t\t\t\t: " << adlen;
        std::cout << std::endl;
...
void Cube::linearityTestZero()
{
    ...
    /** Constructing the associated data **/
    ad = (uint8_t *)malloc(sizeof(uint8_t) * adlen);
    ...
    for(int i = 0; i < combinations; i++)
    {
        ...
        for(int j = 0; j < lc; j++)
        {
            ...
            if(tempCombination)
                ad[generatedCube[j] / 8] |= (1 << (generatedCube[j] % 8));
            else
                ad[generatedCube[j] / 8] &= (~(1 << (generatedCube[j] % 8)));
        }

        TinyJambu tinyJambu(NROUND1, NROUND2);
        tinyJambu.process_ad(zeroKey, ad, adlen, zeroState);
...
bool Cube::linearityTest()
{
    ...
    for(int i = 0; i < combinations; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            ...
            TinyJambu tinyJambu(NROUND1, NROUND2);
            tinyJambu.process_ad(keys[j], ad, adlen, tempInitialState);
...
void Cube::coefficientGeneration()
{
    ...
    for(int i = 0; i < keystreamNo; i++)
    {
        ...
        {
            /** Updating the associated data **/
            trackCombinations++;
            for(int k = 0; k < lc; k++)
            {
                tempCubeCombination = (trackCombinations >> k) & 0x01;
                if(tempCubeCombination)
                    ad[generatedCube[k] / 8] |= (1 << (generatedCube[k] % 8));
                else
                    ad[generatedCube[k] / 8] &= (~(1 << (generatedCube[k] % 8)));
            }
            ...
            TinyJambu tinyJambu(NROUND1, NROUND2);
            tinyJambu.process_ad(key, ad, adlen, tempInitialState);

...
struct TinyJambu {
    ...
    uint32_t process_ad(const uint8_t *, const uint8_t *, uint64_t, uint32_t *);
    uint32_t process_plaintext(const uint8_t *, const uint8_t *, uint64_t, uint32_t *);
...
uint32_t TinyJambu::state_update(uint32_t *state, const uint8_t *key, uint32_t number_of_steps)
{
	...
	for (i = 0; i < (number_of_steps >> 5); i++)
	{
		...
		state[3] = feedback ;
//		printf("\ni: %d\tkeystream: %08X", i, state[2]);
////		printf("\nstate:");
////		for(uint32_t j = 0; j < 4; j++)
////            printf("\t%08X", state[j]);
	}

	if(number_of_steps & 31)
    {
        uint32_t remainderRounds = number_of_steps & 31;
        uint32_t nextWantedBits = 0;

        /** Calculating the feedback (will not modify states) **/
        t1 = (state[1] >> 15) | (state[2] << 17);  // 47 = 1*32+15
		t2 = (state[2] >> 6)  | (state[3] << 26);  // 47 + 23 = 70 = 2*32 + 6
		t3 = (state[2] >> 21) | (state[3] << 11);  // 47 + 23 + 15 = 85 = 2*32 + 21
		t4 = (state[2] >> 27) | (state[3] << 5);   // 47 + 23 + 15 + 6 = 91 = 2*32 + 27
        ...
        /** Shifting the bits according to the remainder number of rounds **/
        for(i = 0; i < 3; i++)
        {
            state[i] >>= remainderRounds;
            nextWantedBits = state[i + 1] & ~((uint32_t) -1 << remainderRounds);
            state[i] |= (nextWantedBits << (32 - remainderRounds));
        }

        /** Putting the feedback for the remaining rounds into the last block of state; **/
        state[3] >>= remainderRounds;
        nextWantedBits = feedback & ~((uint32_t) -1 << remainderRounds);
        state[3] |= (nextWantedBits << (32 - remainderRounds));
    }
...
uint32_t TinyJambu::process_ad(const uint8_t *k, const uint8_t *ad, uint64_t adlen, uint32_t *state)
{
//	uint64_t i;
//	uint32_t j;
...
uint32_t TinyJambu::process_plaintext(const uint8_t *k, const uint8_t *m, uint64_t mlen, uint32_t *state)
{
    int i, j;
    uint32_t ks;
...
//			c[(i << 2) + j] = ((uint8_t*)state)[8 + j] ^ m[(i << 2) + j];
		...
		state[1] ^= mlen & 3;
		ks = state[2];
		state_update(state, k, NROUND2);        // Won't update in last round according to SageMath implementation
	}
...
//	uint32_t j;
    ...
    process_plaintext(k, m, mlen, state);
